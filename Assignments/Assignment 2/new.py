...
for k in range(1,12):
    ...
    graphs.append(Gr)
#CREATE A LIST WHERE WE STORE THE INDEX OF THE FIRST NON-ZERO COLUMN WE ENCOUNTER IN EACH FILE
column = []
for f in range(12):
    for i in range(10):
        for j in range(80):
...
for i in range(1,12):
    ...
    for f in range(10):
        for h in range(80):
            for j in range(10):
                for k in range(80):
                    if png_norm[0][f][h] != 0 or png_norm[i-1][j][k] != 0:
                        gap.add_edge(('1',f,h),(f"{i+1}",j,k),weight = (col[i] - col[0])%80)
